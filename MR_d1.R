...
# 
# keyword
m_temp <- m3[,c('id', 'title', 'release_year','genres1', 'overview', 'tagline')]
...
# row.names(sim_df) <- titles
content_recommender <- function(title, rec_num=10){
  sim_scores <- as.vector(t(sim_df[which(titles==title),]))
  sim_ranks <- order(sim_scores)
  rec_idx <- which(sim_ranks>=2 & sim_ranks<=rec_num+1)
  #rec_titles <- titles[rec_idx]
...
UBCF_Recommender <- function(userId, rec_num=10){
...
hybrid_recommender <- function(userId, title, rec_num){
  sim_scores <- as.vector(t(sim_df[which(titles==title),]))
  sim_ranks <- order(sim_scores)
  rec_idx <- which(sim_ranks>=2 & sim_ranks<=101)
...
hybrid_recommender(55, titles[100], 5)
